...
func main() {
	...
	allMeals, err := d.GetAllMeals(context.TODO())
	if err != nil {
		panic(err)
	}

	fmt.Printf("Fetched %d meals\n", len(allMeals))

	// iterate over all and update the TotalEffort value, which is the mean value of WashingEffort,PeelingEffort,CuttingEffort
	for _, meal := range allMeals {
		totalEffort := (meal.WashingEffort + meal.PeelingEffort + meal.CuttingEffort) / 3
		// round the value down to the neared int (not float!)
		totalEffort = int64(math.Floor(float64(totalEffort)))
		fmt.Println(totalEffort)
		totalTime := meal.CookTime + meal.PrepTime
		...
		if err != nil {
			fmt.Printf("Error updating meal: %v\n", err)
			return
		}
		fmt.Printf("Updated meal %s\n", meal.Name)
	}
